defmodule Ugwu.SocketHandler do
  ...
  @auth_timeout Application.compile_env(:egusi, :websocket_auth_timeout)
  ...
  def websocket_handle({:text, command_json}, state) do
    with {:ok, message_map!} <- Jason.decode(command_json),
      ...
      {:error, changeset = %Ecto.Changeset{}} ->
        %{errors: Egusi.Utils.Errors.changeset_errors(changeset)}
        |> prepare_socket_msg(state)
        |> ws_push(state)
    end
  end
  ...
  defp to_map(changeset = %Ecto.Changeset{}) do
    Egusi.Utils.Errors.changeset_errors(changeset)
  end
